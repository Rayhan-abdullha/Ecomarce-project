...
const FilterByComponents = ({ setChecke, check }: PropTypes) => {
    const url = useParams()
    const productCat: Category | undefined = useSelector((state: RootState) => state.categoriesList.catItems).find(item => item.cat.toString() === url.category?.toString())
    ...
    return (
        <div>
            {Object.keys(productCat?.components || {})?.map((key: string, index: number) => (
                <div key={index} className="bg-white p-4 mb-2 shadow-sm">

                    <h2 className="font-semibold mb-2 text-[17px]">{productCat?.components[key].name}</h2>
                    <div className="flex flex-col gap-3">
                        {productCat?.components[key].list.map((item: string, index: number) => (
